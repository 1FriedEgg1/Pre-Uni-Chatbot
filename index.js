...
client.once(Events.ClientReady, async () => {
  try {
    ...
    console.log(
      `Connected to guild ${client.guilds.cache
    ...
    console.error(err);
    throw new Error(
      "Invalid channel ID in .env file. Please add one and try again."
    );
  }
});
...
client.on(Events.MessageCreate, async (message) => {
  ...
  if (match) {
    ...
    if (equation == "help") {
      message.reply(
        `To use the LaTeX feature, type your equation between two double dollar signs. For example, to render the equation x^2, type $$x^2$$.\n Refer to the LaTeX documentation for more information on syntax.`
      );
      return;
    }
    ...
    if (equation.length > maxEquationLength) {
      message.reply(
        `Equation is too long. It should be less than ${maxEquationLength} characters.`
      );
      return;
    }

    const rendered = await katex.renderToString(equation);
    const buffer = await sharp(Buffer.from(rendered)).png().toBuffer();
    const attachment = new MessageAttachment(buffer, "latex.png");
  ...
  if (
    ...
    if (message.content.replace(`<@${client.user.id}>`, "").length > 1024) {
      return message.reply(
        `\`Please keep your message under 1024 characters. If you need to send a longer message, consider splitting it into multiple messages.\``
      );
    }
    ...
    try {
      ...
      let logEmbed = new EmbedBuilder()
        ...
        .addFields(
          ...
          {
            name: "Output",
            value: `${response.substring(0, 1020)}${
...
function handleOpenAIError(err, message) {
  ...
  let logEmbed = new EmbedBuilder()
    ...
    .setColor("Red")
    .setDescription(
      `An unexpected error occurred. Please see below for details`
    )
    .setAuthor({
      name: `${message.author.tag}`,
      iconURL: `${message.author.displayAvatarURL()}`,
